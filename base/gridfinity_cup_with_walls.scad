...
module gridfinity_cup_with_walls(rows = 1, columns = 1, layers = 3, use_stacking_lip = true, use_magnets = true)
{
    ...
    walls_height = (layers - 1) * 7 - floor_height;
    walls_thickness = 2;
    walls_radius = 4;
    ...
    translate([ 0, 0, -7 ])
    {
            ...
            translate([ 0, 0, -floor_height ])
            {
                ...
                translate([ -(width - cup_base_top_size) / 2, -(length - 41.75) / 2 ])
                {
                    for (row = [0:rows - 1])
                    {
                        ...
                        {
                            translate([ row * 42, column * 42, 0 ]) gridfinity_cup_base(magnets = use_magnets);
                        }
                    }
                }
        ...
        {
            linear_extrude(7) rounded_hollow_square(outer_size = [ width, length ], thickness = walls_thickness,
                                                    radius = walls_radius);
        }
    }
...
gridfinity_cup_with_walls(1, 1, 6);
